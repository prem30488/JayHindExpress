...
public class LinkDAOImpl extends HibernateDaoSupport implements LinkDAO{
	...
	@Override
	@Transactional
	public Link findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Link dept = (Link) session.get(Link.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	@Override
	@Transactional
	public DataSet<Link> findLinkWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<Link> Links = findLinksCriterias(criterias);
		Long count = getTotalCount();
		Long countFiltered = getFilteredCount(criterias);

		return new DataSet<Link>(Links, count, countFiltered);
	}
	...
	public List<Link> list(){
		...
		List<Link> list = (List<Link>) q.list();
		tx.commit();
		session.close();
		return list;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Link> list(Long start,Long end){
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM Link d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Link d");
	...
	@SuppressWarnings("unchecked")
	private List<Link> findLinksCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  Link d");
	...
	public Boolean IsExist(String Link) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Link d where LOWER(Link)=LOWER('"+Link+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<Link> getItemByModel(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Link order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Link>) q.list();
		tx.commit();
		session.close();
	...
	public List<Link> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Link order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Link>) q.list();
		tx.commit();
	...
	public List<Link> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Link where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Link>) q.list();
		tx.commit();
