...
public class AstroDAOImpl extends HibernateDaoSupport implements AstroDAO{
	...
	@Override
	@Transactional
	public Astro findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Astro dept = (Astro) session.get(Astro.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	@Override
	@Transactional
	public DataSet<Astro> findAstroWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<Astro> Astros = findAstrosCriterias(criterias);
		Long count = getTotalCount();
		Long countFiltered = getFilteredCount(criterias);

		return new DataSet<Astro>(Astros, count, countFiltered);
	}
	
	@Override
	@SuppressWarnings("unchecked")
	public List<Astro> list(){
		Session session= HibernateUtil.getSessionFactory().openSession();
		session.setFlushMode(FlushMode.ALWAYS);
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("from Astro");
		List<Astro> list = (List<Astro>) q.list();
		tx.commit();
		session.close();
		return list;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Astro> list(Long start,Long end){
		String hql="from Astro";
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM Astro d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Astro d");
	...
	@SuppressWarnings("unchecked")
	private List<Astro> findAstrosCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  Astro d");
	...
	@Override
	public Boolean IsExist(String Astro) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Astro d where LOWER(Astro)=LOWER('"+Astro+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<Astro> getItemByModel(int page, int recordsPerPage) {
		List<Astro> list = new ArrayList<Astro>();
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Astro order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Astro>) q.list();
		tx.commit();
		session.close();
	...
	public List<Astro> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Astro order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Astro>) q.list();
		tx.commit();
	...
	public List<Astro> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Astro where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Astro>) q.list();
		tx.commit();
