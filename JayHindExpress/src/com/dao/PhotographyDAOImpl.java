...
public class PhotographyDAOImpl extends HibernateDaoSupport implements PhotographyDAO{

	public PhotographyDAOImpl(){
		setSessionFactory(HibernateUtil.getSessionFactory());
	}
	
	@Override
	@Transactional
	public void delete(Photography dept) {
	...
	@Override
	@Transactional
	public Photography findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Photography dept = (Photography) session.get(Photography.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	public Boolean updateFrequency(Long deptId) {
		Photography p = findByDeptId(deptId);
	...
	@Override
	@Transactional
	public DataSet<Photography> findPhotographyWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<Photography> Photographys = findPhotographysCriterias(criterias);
		Long count = getTotalCount();
		Long countFiltered = getFilteredCount(criterias);

		return new DataSet<Photography>(Photographys, count, countFiltered);
	}
	
	@Override
	@SuppressWarnings("unchecked")
	public List<Photography> list(){
		Session session= HibernateUtil.getSessionFactory().openSession();
		session.setFlushMode(FlushMode.ALWAYS);
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("from Photography");
		List<Photography> list = (List<Photography>) q.list();
		tx.commit();
		session.close();
		return list;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Photography> list(Long start,Long end){
		String hql="from Photography";
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM Photography d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Photography d");
	...
	@SuppressWarnings("unchecked")
	private List<Photography> findPhotographysCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  Photography d");
	...
	@Override
	@Transactional
	public void save(Photography dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		session.save(dept);
		tx.commit();
	...
	@Override
	@Transactional
	public void update(Photography dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	@Override
	public Boolean IsExist(String Photography) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Photography d where LOWER(Photography)=LOWER('"+Photography+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<Photography> getItemByModel(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Photography order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Photography>) q.list();
		tx.commit();
		session.close();
	...
	public List<Photography> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Photography order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Photography>) q.list();
		tx.commit();
	...
	public List<Photography> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Photography where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Photography>) q.list();
		tx.commit();
