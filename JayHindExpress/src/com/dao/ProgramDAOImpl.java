...
public class ProgramDAOImpl extends HibernateDaoSupport implements ProgramDAO{
	...
	@Override
	@Transactional
	public void delete(Program dept) {
	...
	@Override
	@Transactional
	public Program findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Program dept = (Program) session.get(Program.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	@Override
	@Transactional
	public DataSet<Program> findProgramWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<Program> programs = findProgramsCriterias(criterias);
		Long count = getTotalCount();
		Long countFiltered = getFilteredCount(criterias);

		return new DataSet<Program>(programs, count, countFiltered);
	}
	...
	public List<Program> list(){
		...
		List<Program> list = (List<Program>) q.list();
		tx.commit();
		session.close();
		return list;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Program> list(Long start,Long end){
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM Program d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Program d");
	...
	@SuppressWarnings("unchecked")
	private List<Program> findProgramsCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  Program d");
	...
	@Override
	@Transactional
	public void update(Program dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	public Boolean IsExist(String program) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Program d where LOWER(program)=LOWER('"+program+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<Program> getItemByModel(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Program order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Program>) q.list();
		tx.commit();
		session.close();
	...
	public List<Program> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Program order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Program>) q.list();
		tx.commit();
	...
	public List<Program> getItemByCityFrequency(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Program where lower(location) like '%"+ city.toLowerCase() +"%' order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Program>) q.list();
		tx.commit();
	...
	public List<Program> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Program where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Program>) q.list();
		tx.commit();
