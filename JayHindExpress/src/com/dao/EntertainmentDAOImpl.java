...
public class EntertainmentDAOImpl extends HibernateDaoSupport implements EntertainmentDAO{
	...
	@Override
	@Transactional
	public void delete(Entertainment dept) {
	...
	@Override
	@Transactional
	public Entertainment findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Entertainment dept = (Entertainment) session.get(Entertainment.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	public Boolean updateFrequency(Long deptId) {
		Entertainment p = findByDeptId(deptId);
	...
	@Override
	@Transactional
	public DataSet<Entertainment> findEntertainmentWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<Entertainment> Entertainments = findEntertainmentsCriterias(criterias);
		Long count = getTotalCount();
		Long countFiltered = getFilteredCount(criterias);

		return new DataSet<Entertainment>(Entertainments, count, countFiltered);
	}
	...
	public List<Entertainment> list(){
		...
		session.setFlushMode(FlushMode.ALWAYS);
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("from Entertainment");
		List<Entertainment> list = (List<Entertainment>) q.list();
		tx.commit();
		session.close();
		return list;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Entertainment> list(Long start,Long end){
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM Entertainment d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Entertainment d");
	...
	@SuppressWarnings("unchecked")
	private List<Entertainment> findEntertainmentsCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  Entertainment d");
	...
	@Override
	@Transactional
	public void save(Entertainment dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		session.save(dept);
		tx.commit();
	...
	@Override
	@Transactional
	public void update(Entertainment dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	@Override
	public Boolean IsExist(String Entertainment) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Entertainment d where LOWER(Entertainment)=LOWER('"+Entertainment+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<Entertainment> getItemByModel(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Entertainment order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Entertainment>) q.list();
		tx.commit();
		session.close();
	...
	public List<Entertainment> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Entertainment order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Entertainment>) q.list();
		tx.commit();
	...
	public List<Entertainment> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Entertainment where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Entertainment>) q.list();
		tx.commit();
