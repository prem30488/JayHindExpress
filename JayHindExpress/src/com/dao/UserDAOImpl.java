...
public class UserDAOImpl extends HibernateDaoSupport implements UserDAO{
	...
	@Override
	@Transactional
	public User findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		User dept = (User) session.get(User.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	@Override
	@Transactional
	public DataSet<User> findUserWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<User> programs = findUsersCriterias(criterias);
		
		Iterator<User> iter = programs.iterator();
		...
		Long count = getTotalCount()-1;
		Long countFiltered = getFilteredCount(criterias)-1;

		return new DataSet<User>(programs, count, countFiltered);
	}
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM User d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM User d");
	...
	@SuppressWarnings("unchecked")
	private List<User> findUsersCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  User d");
	...
	public Boolean IsExist(String program) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM User d where LOWER(program)=LOWER('"+program+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<User> getItemByModel(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM User order by createdDate desc,frequency desc" );
	...
	public List<User> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM User order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<User>) q.list();
		tx.commit();
	...
	public List<User> getItemByCityFrequency(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM User where lower(location) like '%"+ city.toLowerCase() +"%' order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<User>) q.list();
		tx.commit();
	...
	public List<User> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM User where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<User>) q.list();
		tx.commit();
