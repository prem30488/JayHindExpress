...
public class TechnologyDAOImpl extends HibernateDaoSupport implements TechnologyDAO{
	...
	@Override
	@Transactional
	public void delete(Technology dept) {
	...
	@Override
	@Transactional
	public Technology findByDeptId(Long deptId) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Technology dept = (Technology) session.get(Technology.class, deptId);
		tx.commit();
		session.close();
		return dept;
	}
	...
	public Boolean updateFrequency(Long deptId) {
		Technology p = findByDeptId(deptId);
	...
	@Override
	@Transactional
	public DataSet<Technology> findTechnologyWithDatatablesCriterias(
			DatatablesCriterias criterias) {
		List<Technology> Technologys = findTechnologysCriterias(criterias);
		Long count = getTotalCount();
		Long countFiltered = getFilteredCount(criterias);

		return new DataSet<Technology>(Technologys, count, countFiltered);
	}
	...
	public List<Technology> list(){
		...
		session.setFlushMode(FlushMode.ALWAYS);
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("from Technology");
		List<Technology> list = (List<Technology>) q.list();
		tx.commit();
		session.close();
		return list;
	}
	...
	@Override
	@SuppressWarnings("unchecked")
	public List<Technology> list(Long start,Long end){
	...
	private Long getFilteredCount(DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d FROM Technology d");
	...
	private Long getTotalCount() {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Technology d");
	...
	@SuppressWarnings("unchecked")
	private List<Technology> findTechnologysCriterias(
			DatatablesCriterias criterias) {
		StringBuilder queryBuilder = new StringBuilder("SELECT d from  Technology d");
	...
	@Override
	@Transactional
	public void save(Technology dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		session.save(dept);
		tx.commit();
	...
	@Override
	@Transactional
	public void update(Technology dept) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
	...
	@Override
	public Boolean IsExist(String Technology) {
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("SELECT COUNT(d) FROM Technology d where LOWER(Technology)=LOWER('"+Technology+"')");
		Long count = (Long) q.list().get(0);
		tx.commit();
		session.close();
		if(count>0)
	...
	public List<Technology> getItemByModel(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Technology order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Technology>) q.list();
		tx.commit();
		session.close();
	...
	public List<Technology> getItemByFrequency(int page, int recordsPerPage) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Technology order by frequency desc, createdDate desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Technology>) q.list();
		tx.commit();
	...
	public List<Technology> getItemByCity(int page, int recordsPerPage, String city) {
		...
		Session session= HibernateUtil.getSessionFactory().openSession();
		Transaction tx = session.beginTransaction();
		Query q = session.createQuery("FROM Technology where lower(location) like '%"+city.toLowerCase()+"%' order by createdDate desc,frequency desc" );
		q.setFirstResult(page); 
        q.setMaxResults(recordsPerPage);
		list = (List<Technology>) q.list();
		tx.commit();
